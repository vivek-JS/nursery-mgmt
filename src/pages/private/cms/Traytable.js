...
const TrayTable = () => {
  ...
  const getTrays = async () => {
    ...
    try {
      ...
      if (response.data?.data) {
        setTrays(response.data.data.data)
  ...
  const handleStatusToggle = async (id, currentStatus) => {
    setLoading(true)
    try {
      const instance = NetworkManager(API.TRAY.TOGGLE_STATUS)
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {trays.map((tray) => (
              <tr key={tray._id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <button
                    ...
                    className={`px-3 py-1 rounded-full text-sm font-medium ${
                      tray.isActive ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"
                    }`}>
        ...
        <form onSubmit={handleSubmit}>
          <DialogContent className="mt-4">
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700">Tray Name</label>
                <input
                  type="text"
                  required
                  value={formData.name}
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700">Cavity Count</label>
                <input
                  type="number"
                  required
                  min="1"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700">Number per Crate</label>
                <input
                  type="number"
                  required
                  min="1"
                  step="1"
                  value={formData.numberPerCrate}
                  onChange={(e) =>
                    setFormData({ ...formData, numberPerCrate: parseInt(e.target.value) })
                  }
                  className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2"
                />
              </div>
