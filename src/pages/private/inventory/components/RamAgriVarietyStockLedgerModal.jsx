...
export default function RamAgriVarietyStockLedgerModal({
  open,
  onClose,
  loading,
  data,
  ...
  const titleCrop = variety?.cropName || pendingLabel?.cropName;
  const titleVariety = variety?.varietyName || pendingLabel?.varietyName;
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
      ...
      <div className="relative z-10 flex max-h-[90vh] w-full max-w-6xl flex-col overflow-hidden rounded-xl bg-white shadow-2xl">
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-gray-200 bg-white p-6">
          <div>
            <h2 className="text-2xl font-bold text-gray-800">Variety stock ledger</h2>
            <p className="mt-1 text-gray-600">
              {titleCrop && titleVariety
                ? `${titleCrop} — ${titleVariety}`
                : titleCrop || titleVariety || "Loading variety…"}
            </p>
            <p className="mt-0.5 text-xs text-gray-500">All-time stock IN/OUT history (not limited by dashboard date filter)</p>
            {data?.ledgerSource && (
              <p className="mt-0.5 text-xs text-gray-500">
                Source: {data.ledgerSource === "movements" ? "movement history" : "legacy computed"}
              </p>
            )}
          </div>
          <button
                          ...
                          <React.Fragment key={`${entry.reference}-${index}`}>
                            <tr className="hover:bg-gray-50">
                              ...
                              <td className="px-4 py-3 text-sm font-medium text-gray-800">
                                {entry.category || entry.movementType || "—"}
                              </td>
                                  ...
                                  <div className="ml-6 flex flex-wrap gap-2">
                                    {batches.map((b, bi) => (
                                      <span
                                        ...
                                        className="inline-flex items-center gap-1 rounded-md border border-slate-200 bg-white px-2 py-1 text-xs tabular-nums text-slate-700"
                                      >
