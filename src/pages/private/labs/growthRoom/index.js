...
import GrowthRoomModal from './growthRoomModal';
...
const GrowthRoom = () => {
    ...
    const [growthRoomEntries, setGrowthRoomEntries] = useState([
        { id: 1, numberOfEmployeeEngaged: 5, listOfEmployeeEngaged: 'Alice, Bob', in: 2, out: 3 },
        { id: 2, numberOfEmployeeEngaged: 3, listOfEmployeeEngaged: 'John, Jane', in: 1, out: 2 }
    ]);
    ...
    const handleOpenModal = (index = null) => {
        if (index !== null) {
            setFormData(growthRoomEntries[index]);
        } else {
            setFormData({
                numberOfEmployeeEngaged: '',
                listOfEmployeeEngaged: '',
    ...
    const handleDelete = (index) => {
        const updatedEntries = growthRoomEntries.filter((_, i) => i !== index);
        setGrowthRoomEntries(updatedEntries);
    };

    const handleSubmit = () => {
        if (editIndex !== null) {
            const updatedEntries = [...growthRoomEntries];
            updatedEntries[editIndex] = { ...formData, id: updatedEntries[editIndex].id };
            setGrowthRoomEntries(updatedEntries);
        } else {
            setGrowthRoomEntries([...growthRoomEntries, { ...formData, id: Date.now() }]);
        }
    ...
    return (
        <Box sx={{ p: 3 }}>
            ...
            <Grid container className={classes.inventoryContainer}>
                ...
                {growthRoomEntries.map((entry, index) => (
